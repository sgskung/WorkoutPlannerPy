...
def good(gst, d, v, w):
    return v not in list(gst.adj[w]) and graph.edges[v, w, 0]['length'] > 0
...
def find_route(start, goal_dist, graph):
    ...
    # find_route(ox.get_nearest_node(graph,(49.255, -123.255)),5000, graph)
    ...
    while stack:
        ...
        if curr not in list(gst.neighbors(prev)):
            ...
            # neighbors = sorted(graph.neighbors(curr)), key = lambda x: get_bearing_diff(graph.edges[prev, curr, 0]['bearing'], graph.edges[curr, x, 0]['bearing'])
            neighbors = sorted(graph.neighbors(curr),
                               key=lambda x: get_bearing_diff(graph.edges[prev, curr, 0]['bearing'],
                                                              graph.edges[curr, x, 0]['bearing']), reverse=True)
...
def total_elevation_gain(gr, rt):
    ...
    for x in range(len(rt)):
        if gr.nodes[rt[x]]['elevation'] > 0:
            if gr.nodes[rt[x]]['elevation'] > gr.nodes[rt[x - 1]]['elevation']:
                eg += gr.nodes[rt[x]]['elevation'] - gr.nodes[rt[x - 1]]['elevation']
    return eg
...
routevertices = [nodes[0] for nodes in sorted_lst]
